...
public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : BaseEntity
{
    ...
    public async Task<IEnumerable<TEntity>> GetMany(int Page, int PageItems, Expression<Func<TEntity, bool>> query)
    {
    ...
    public async Task<int> GetAmount(Expression<Func<TEntity, bool>> query)
    {
        return await context.Set<TEntity>().Where(query).CountAsync();
    }
}